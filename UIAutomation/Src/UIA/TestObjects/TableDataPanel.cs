...
namespace UIAutomation.Src.UIA.TestObjects
{
    ...
    /// </summary>
    public class TableDataPanel : TestObjectBase
    {
        ...
        public TableDataPanel( AutomationElement element ) : base( element )
        {
        }
        ...
        {
            return FindFirstChild<GUIObject>( name: "Data Panel" ).FindFirstChild<TableRow>( name: $"Row {rowIndex}" );
        }
        ...
        {
            return  FindFirstChild<GUIObject>(name: "Data Panel").FindAllChildren<TableRow>().FirstOrDefault( row => row.ContainsValue( value ) );
        }
        ...
        {
            return FindFirstChild<GUIObject>( name: "Data Panel" ).FindAllChildren<TableRow>().FirstOrDefault( row => row.ContainsValueAtColumnIndex( value, columnIndex ) );
        }
        ...
        {
            return FindFirstChild<GUIObject>( name: "Data Panel" ).FindAllChildren<TableRow>().Where( row => row.ContainsValue( value ) );
        }
        ...
        {
            return FindFirstChild<GUIObject>( name: "Data Panel" ).FindAllChildren<TableRow>().Where( row => row.ContainsValueAtColumnIndex( value, columnIndex ) );
        }
        ...
        //    foreach(TableRow row in FindFirstChild<GUIObject>( name: "Data Panel" ).FindAllChildren<TableRow>())
        //    {
        ...
        public List<Header> GetTableHeaders()
        {
            ...
            var row = FindFirstChild<GUIObject>( name: $"Header Panel" );
        ...
        //    int tableColumns = tableRow.FindAllChildren<GUIObject>().Count();
        ...
        public List<string> GetColumnCells(int columnIndex)
        {
            List<string> cellsValue = new List<string>();
            var rows = FindFirstChild<GUIObject>(name: "Data Panel").FindAllChildren<TableRow>();

            foreach (var row in rows)
            {
                cellsValue.Add(row.GetCellOfType<Custom>( columnIndex ).GetValue());
            }
